  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      if (
        ...
        (error && typeof error === 'object' && error.code === -32603 && typeof error.message === 'string' &&
          (error.message.toLowerCase().includes('not found') || error.message.toLowerCase().includes('unknown tool')))
      ) {
        let code = -32001;
        let message = typeof error === 'string' ? error : (error && error.message) || 'SDK validation error';
        if (
